...
public class Cengkeh extends Tanaman implements TreatmentInterface {
    ...
    public double dailyProfit(){
        // Code here
        return dailySell()-getExpensePerDay();
    }
    ...
    @Override public String toString(){
        return super.toString()+
                String.format("Cengkeh/hari : "+ resultInKgPerDay+" kg (per hektar)\n" +
                "Harga pasar  : Rp. %,.2f (per kg)\n" +
                "Penjualan    : Rp. %,.2f (per hari)\n" +
                "Profit       : Rp. %,.2f (per hari)\n" +
                "Status       : " + getStatus(), cengkehPrice, dailySell(), dailyProfit());
    }
